...
export function DataFetcher(props: { children: any }) {
  ...
  useEffect(() => {
    ...
    if (isNotStarted(arbeidsgivere)) {
      arbeidsgivere.fetch(env.syfoapiRoot + '/syfosoknad/api/soknader', {
        credentials: 'include',
      }, (fetchState: FetchState<Organisasjon[]>) => {
        if (hasData(fetchState)) {
          setArbeidsgivere(fetchState.data);
        }
      })
    }
  ...
  if (isAnyNotStartedOrPending([ unleash, arbeidsgivere ])) {
    ...
    window.location.href = `${hentLoginUrl()}?redirect=${window.location.origin}/contextroot`;
