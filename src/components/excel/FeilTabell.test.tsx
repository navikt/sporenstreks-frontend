...
describe('FeilTabell', () => {
  ...
  it('should show nothing when given no errors', () => {
    ...
    expect(
      screen.queryByText(/Følgende feil i dokumentet må utbedres/)
  ...
  it('should show a simple table when given <10 errors', () => {
    ...
    expect(
      view.getByText(/Følgende feil i dokumentet må utbedres/)
    ).toBeInTheDocument();

    expect(
      view.queryByText(/Vis alle rader med feilmelding/)
    ).not.toBeInTheDocument();
    expect(
      view.queryByRole('button', { name: /Vis feilmeldingsammendrag/ })
    ).not.toBeInTheDocument();
  });
  ...
  it('should show a summary when given >10 errors', () => {
    ...
    expect(
      view.getByText(/15 feil i dokumentet må utbedres/)
    ).toBeInTheDocument();
    expect(
      view.getByRole('button', { name: /Vis alle rader med feilmelding/ })
    ).toBeInTheDocument();
  });
  ...
  it('should show a full list when given >10 errors and visAlleFeil', () => {
    ...
    expect(
      view.getByRole('button', { name: /Vis feilmeldingsammendrag/ })
    ).toBeInTheDocument();

    expect(
      view.queryByText(/15 feil i dokumentet må utbedres/)
    ).not.toBeInTheDocument();
    expect(
      view.queryByRole('button', { name: /Vis alle rader med feilmelding/ })
    ).not.toBeInTheDocument();
  });
  ...
  it('should toggle between summary and full list', () => {
    ...
    const toggleButton = screen.getByRole('button', {
      name: /Vis alle rader med feilmelding/
    });

    userEvent.click(toggleButton);

    expect(
      screen.getByRole('button', { name: /Vis alle rader med feilmelding/ })
    ).toBeInTheDocument();
