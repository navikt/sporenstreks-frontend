  ...
  const gruppertFeil = feil.reduce(function (gruppert, feil) {
    ...
    if (feilGruppering) {
      ...
      gruppert.push({
        ...
        kolonne: feil.kolonne
      });
    }
    return gruppert;
  }, [] as tabellFeil[]);

  const tabellSortAscending = (x: tabellFeil, y: tabellFeil) =>
    x.indeks - y.indeks;

  const tabellSortDescending = (x: tabellFeil, y: tabellFeil) =>
    y.indeks - x.indeks;
    ...
    return (
      <div className='feilvisningstabell feiloppsummering'>
        <div className='tabell--overflow'>
          <Ingress>
            Følgende feil i dokumentet må utbedres før du laster det opp på
            nytt:
          </Ingress>
        ...
        <Vis hvis={feil.length > 10}>
          <Knapp
            className='feilvisningstabell__knapp'
            onClick={toggleFeilvisning}
          >
            Vis feilmeldingsammendrag
          </Knapp>
        </Vis>
      </div>
    );
  ...
  const feilvisningsTabellGruppert = (gruppertFeil: tabellFeil[]) => {
    return (
      <div className='feilvisningstabell feiloppsummering'>
        {/* eslint-disable-next-line react/prop-types */}
        <Ingress>
          {feil.length} feil i dokumentet må utbedres før du laster det opp på
          nytt:
        </Ingress>
        <div className='tabell--overflow'>
          <table className='tabell tabell--stripet'>
            ...
            <tfoot>
              <tr>
                <th>
                  <Knapp onClick={toggleFeilvisning}>
                    Vis alle rader med feilmelding
                  </Knapp>
                </th>
              </tr>
            </tfoot>
  ...
  return (
    <Vis hvis={feil.length > 0}>
      {feil.length <= 10 || visAlleFeil
        ? feilvisningsTabellVanlig()
        : feilvisningsTabellGruppert(gruppertFeil)}
    </Vis>
  );
};
