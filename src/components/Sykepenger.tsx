...
const Sykepenger = () => {
  ...
  const findError = (felt: string): ErrorObject => {
    ...
    return err ?? { fieldName: felt, errorMessage: '' };
  };
    ...
    await fetch(process.env.REACT_APP_BASE_URL + '/api/v1/refusjonskrav', {
      ...
      if (response.status === 401) {
        ...
        response.json().then(data => {
          console.log('data-422', data); // eslint-disable-line
        ...
        setErrors([ { errorType: ErrorType.UNKNOWN, errorMessage: 'Feil ved innsending av skjema' } ])
      }
    });
  };
